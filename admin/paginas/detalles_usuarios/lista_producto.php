...
try {
    ...
    if (!verificarEntradaDatosSession(['id_usuario_administrador', 'tipo_usuario_admin'])) {
        throw new Exception("Debe iniciar sesion para poder la lista de productos del usuario");
    }
    ...
    if (!verificarSiEsUsuarioAdminValido($conexion, $id_usuario_administrador, $tipo_usuario_admin)) {
        throw new Exception("No se puede ver la lista de productos por que no es usuario administrador valido");
    }
    ...
    $condicion_where = obtenerCondicionWhereBuscadorProductoParaAdmin($campo_buscar, $fecha, $categoria, $estado);
    ...
    $respuesta['card_productos'] = listaCardProductosEmprendedorParaAdmin($conexion, $lista_productos, $busqueda_activa);
...
function listaCardProductosEmprendedorParaAdmin($conexion, $lista_productos, $busqueda_activa)
{
    $datos = '';
        ...
        for ($i = 0; $i < count($lista_productos); $i++) {
            ...
            $datos .= "<div class='col-12 col-sm-12 col-md-12 col-lg-10 mb-3 mt-2'>";
                            ...
                            $datos .= "<div class='col-12 col-sm-12 col-md-8 col-lg-8'>";
                                ...
                                $datos .= "<div class='row'>";
                                    ...
                                    if ($lista_productos[$i]['fecha_modificación'] != null) {
                                        ...
                                        $datos .= "<div class='col-6 col-sm-6 col-md-6 col-lg-6'>";
                                            $datos .= "<p class='card-text'><strong>Fecha de modificacion:</strong>{$fecha_modificacion}</p>";
                                        $datos .= "</div>";
                                    }
                ...
                if ($lista_productos[$i]['id_estado_producto'] != 2) {
                    ...
                    $datos .= '<a class="btn btn-outline-success m-2" target="_blank"  href="'.$url_base_archivos.'/admin/paginas/detalles_usuarios/producto/pagina_detalles_producto.php?id=' . $lista_productos[$i]['id_publicacion_producto'] . '&token=' . hash_hmac('sha1', $lista_productos[$i]['id_publicacion_producto'], KEY_TOKEN) . '">Vista administrador</a>';
                    $datos .= '<a class="btn btn-outline-warning m-2" target="_blank"  href="'.$url_base_archivos.'/admin/paginas/detalles_usuarios/producto/pagina_modificar_producto.php?id_usuario='.$lista_productos[$i]['id_usuario'].'&id_producto=' . $lista_productos[$i]['id_publicacion_producto'] . '&token=' . hash_hmac('sha1', $lista_productos[$i]['id_publicacion_producto'], KEY_TOKEN) . '">Modificar</a>';
                    $datos .= "<button type='button' class='btn btn-outline-danger m-2' data-bs-dismiss='modal' data-bs-id_producto='{$lista_productos[$i]['id_publicacion_producto']}'  data-bs-nombre_producto='{$lista_productos[$i]['nombre_producto']}' data-bs-toggle='modal' data-bs-target='#ModaleliminarProducto'>Eliminar</button>";
                    ...
                    if (count($preguntasGenerales) > 0) {
                        $datos .= '<button type="button" class="btn btn-outline-primary m-2" data-bs-dismiss="modal" data-bs-id_producto="' . $lista_productos[$i]['id_publicacion_producto'] . '" data-bs-toggle="modal" data-bs-target="#preguntasModal-' . $lista_productos[$i]['id_publicacion_producto'] . '">Preguntas</button>';
                        $datos .= obtenerModalListaPreguntasRespuestaParaAdmin($preguntasGenerales, $lista_productos[$i]['id_publicacion_producto']);
                    }
...
function obtenerModalListaPreguntasRespuestaParaAdmin($preguntas, $id)
{
    $datos = '';
