...
$id_usuario_admin = isset($_GET['id']) ? $_GET['id'] : '';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST)) {
        ...
        if (empty($id_usuario_admin)  || empty($token)) {
        ...
        if (!validarTokenNuevaPasswordAdmin($conexion, $token,$id_usuario_admin)) {
        ...
        $datos_usuario = obtenerDatosUsuarioAdministrador($conexion, $id_usuario_admin);
        $email =  $datos_usuario['email'];

        //Verifica si la cuenta del usuario es valido por email
        if (!verificarSiEsUsuarioValidoPorEmailAdmin($conexion, $email)) {
            throw new Exception("No se puede modificar la contraseña del usuario debido a que la cuenta no esta activa");
        }
        ...
        //Modificar la contraseña actual de usuario por la nuea contraseña ingresada
        modificarPasswordOlvidadoUsuarioAdmin($conexion, $nueva_password, $token, $email);
